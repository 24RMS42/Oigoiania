...
import com.jwetherell.augmented_reality.activity.MyTempDataHolder;
...
public class Marker implements Comparable<Marker>, Serializable {
	...
	public String id = null; // location id//
	...
	private MyTempDataHolder data = null;
	...
	public Marker(String name, double latitude, double longitude,
		...
		data = new MyTempDataHolder();
		Log.d("test", "Marker : Namse :" + name + " Latitude :" + latitude
	...
	protected synchronized void drawIconNew(Canvas canvas) {
		if (canvas == null)
			throw new NullPointerException();

		if (gpsSymbol == null) {
			Log.d("IconMarker", "again null");
		...
		if (AugmentedRealityUtil.portrait) {
			...
			x -= textBox.getWidth() / 2;
			Log.d("IconMarker", "else ->Landscape Y2= " + y);
			Log.d("IconMarker", "else ->Landscape X1= " + x);
		}
		float currentAngle = 0;
		if (AugmentedRealityUtil.portrait)
			currentAngle = -90;
		...
		if (symbolContainer == null)
			symbolContainer = new PaintablePosition(gpsSymbol, x, y + (y / 2),
					currentAngle, 1);
		else
			symbolContainer.set(gpsSymbol, x, y + (y / 2), currentAngle, 1);
...
//		Log.d("test", "Marker : text size :" + (Math.round(maxHeight / 2f) + 1)
//				+ " maxHeight" + maxHeight);
		if (textBox == null)
			textBox = new PaintableBoxedText(textStr, 9, 300, bitmap);
		...
		// textContainer.paintRect(canvas, x, y, textBox.getWidth(),
		// textBox.getHeight());
	...
	public MyTempDataHolder getData() {
		return this.data;
	}

	public void setData(MyTempDataHolder data) {
		this.data = data;
	}
