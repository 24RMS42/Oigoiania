...
import com.jwetherell.augmented_reality.ui.objects.PaintableRadarPoint;
...
public class Radar {
	...
	private PaintableRadarPoint radarPoint = null;
	 ...
	 *             if Canvas is NULL.
	 */
	public void draw(Canvas canvas, Marker marker) {
		if (canvas == null)
			throw new NullPointerException();
		...
		// bearing
		drawRadarCircle(canvas);
		drawRadarPoint(canvas, marker);
		drawRadarLines(canvas);
		//drawRadarText(canvas, marker);
		drawRadarText(canvas);
	...
	private void drawRadarPoints(Canvas canvas) {
		...
		//if (!AugmentedRealityUtil.isLargeSize()) {
			// old
			if (pointsContainer == null)
				pointsContainer = new PaintablePosition(radarPoints, PAD_X,
						PAD_Y, -ARData.getAzimuth(), 1);
			else
				pointsContainer.set(radarPoints, PAD_X, PAD_Y,
						-ARData.getAzimuth(), 1);
//		} else {
//			// New
//			Log.d("azimuth", "Azimuth is : " + ARData.getAzimuth());
//			if (pointsContainer == null)
//				pointsContainer = new PaintablePosition(radarPoints, PAD_X,
//						PAD_Y, -(ARData.getAzimuth() - 160), 1);
//			else
//				pointsContainer.set(radarPoints, PAD_X, PAD_Y,
//						-(ARData.getAzimuth() - 160), 1);
//	}
		pointsContainer.paint(canvas);
	}

	private void drawRadarPoint(Canvas canvas, Marker marker) {
		if (canvas == null)
			throw new NullPointerException();

		if (radarPoint == null)
			radarPoint = new PaintableRadarPoint(marker);

		if (pointsContainer == null)
			pointsContainer = new PaintablePosition(radarPoint, PAD_X, PAD_Y,
					-ARData.getAzimuth(), 1);
		else
			pointsContainer.set(radarPoint, PAD_X, PAD_Y, -ARData.getAzimuth(),
					1);
	...
	private void drawRadarText(Canvas canvas, Marker marker) {
		if (canvas == null)
			throw new NullPointerException();
		...
		// Zoom text
		float distance = (float) marker.getDistance();
		radarText(canvas, formatDist(distance), (PAD_X + RADIUS), (PAD_Y
				+ RADIUS * 2 - 10), false);
	}
	...
	private static String formatDist(float meters) {
		if (meters < 1000) {
			...
			return formatDec(meters / 1000f, 2) + "km";
		} else {
